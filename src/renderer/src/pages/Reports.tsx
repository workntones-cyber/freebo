    ...
    <div>
      ...
      <div className="card">
        ...
        <table className="table" style={{ marginBottom: 24 }}>
          ...
          <tbody>
            {revenues.map((r, i) => <tr key={i}><td>{r.account_name}</td><td style={{ textAlign: 'right' }}>{fmt(r.amount)}</td></tr>)}
        ...
        <table className="table" style={{ marginBottom: 24 }}>
          <thead><tr><th>勘定科目</th><th style={{ textAlign: 'right' }}>金額</th></tr></thead>
          <tbody>
            {expenses.map((r, i) => <tr key={i}><td>{r.account_name}</td><td style={{ textAlign: 'right' }}>{fmt(r.amount)}</td></tr>)}
            <tr style={{ fontWeight: 700 }}>
              <td>費用合計</td>
